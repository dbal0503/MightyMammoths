...
function NavigationSheet({
  ...
  const { setSelectedMode, setRouteEstimates, setRoutesValid } = functions;
  ...
  const [isBackgroundInteractionEnabled, setIsBackgroundInteractionEnabled] =
    useState(false);

  const setPoly = (poly: string) => {
    const decodedPoly: LatLng[] = polyline
      .decode(poly)
        ...
        <View style={styles.centeredView}>
          {selectedMode === null || startedSelectedRoute === false ? (
            <TransportChoice
              ...
              routes={
                selectedMode && routeEstimates[selectedMode]
                  ? routeEstimates[selectedMode]
                  : []
              }
              starting={() => {
                closeChooseDest(false);
              ...
              defPoly={() => {
                ...
                if (
                  selectedMode &&
                  routeEstimates[selectedMode]?.length > 0 &&
                  ...
                  const shuttlePolyline =
                    steps.find((step) => step.mode === "BUS")?.polyline || "";
                  ...
                  const combinedPoly = combinePolylines(
                    walk1Polyline,
                    shuttlePolyline,
                    walk2Polyline
                  );
            ...
            <LiveInformation
              ...
              onIndoorMapPress={navigateIndoor}
              routes={routeEstimates[selectedMode] || []}
              onZoomOut={onZoomOut}
